////////////////////////////////////////////////////////////////////////////////////////////////////
/// \file	MediaPlayer\MyWizardPanel.cpp
///
...
:SallyAPI::GUI::CWizardPanel(parent, graphicId, pluginPath)
{
	...
	m_pLabelHeader->SetText("Media Player");

	m_pLabelBoxWelcome1 = new SallyAPI::GUI::CLabelBox(this, 20, 50 + CONTROL_HEIGHT, WIZARD_PANEL_WIDTH - 40, 60);
	m_pLabelBoxWelcome1->SetText("After you have selected your media directories Sally will create the database with all necessary infos.\n" \
		"This can take some time to complete. Before the database create is not finished, the search and the album browser will not show all your media files.");
	m_pLabelBoxWelcome1->SetLocalised(true);
	this->AddChild(m_pLabelBoxWelcome1);

	m_pLabelBoxWelcome2 = new SallyAPI::GUI::CLabelBox(this, 20, 50 + CONTROL_HEIGHT + 10 + 60, WIZARD_PANEL_WIDTH - 40, CONTROL_HEIGHT);
	m_pLabelBoxWelcome2->SetText("Please select your mp3 and video folders:");
	m_pLabelBoxWelcome2->SetLocalised(true);
	this->AddChild(m_pLabelBoxWelcome2);

	for (int i = 0; i < 8; ++i)
	{
		m_pSelection[i] = new SallyAPI::GUI::CEdit(this,
			50, 20 + CONTROL_HEIGHT + 130 + (i * (CONTROL_HEIGHT + 10)), 500, i);
		m_pSelection[i]->SetInfoText("Media Folder");
		this->AddChild(m_pSelection[i]);

		m_pSelectFolder[i] = new SallyAPI::GUI::CButton(this,
			50 + 500 + 10, 20 + CONTROL_HEIGHT + 130 + (i * (CONTROL_HEIGHT + 10)), 130, CONTROL_HEIGHT,
		...
		m_pEmptySelection[i] = new SallyAPI::GUI::CButton(this,
			50 + 500 + 10 + 10 + 130, 20 + CONTROL_HEIGHT + 130 + (i * (CONTROL_HEIGHT + 10)), CONTROL_HEIGHT, CONTROL_HEIGHT,
...
void CMyWizardPanel::SaveConfig()
{
	for (int i = 0; i < 8; i++)
	{
		...
		SetPropertyInt(includeInDatabase, 1);
