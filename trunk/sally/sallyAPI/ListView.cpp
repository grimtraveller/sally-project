...
/// int height, int rows, int actionGraphic, std::vector<int>& pictureList,
/// int controlId) :SallyAPI::GUI::CForm(parent, x, y, width, height, controlId),
/// m_iStartPicture(0), m_iRows(rows), m_iActionGraphic(actionGraphic),
...
CListView::CListView(SallyAPI::GUI::CGUIBaseObject* parent, int x, int y, int width, int height, 
					 int rows, int actionGraphic, std::vector<int>& pictureList, int controlId)
	:SallyAPI::GUI::CForm(parent, x, y, width, height, controlId), m_iStartPicture(0),
	m_iRows(rows), m_iActionGraphic(actionGraphic), m_vPictureList(pictureList), m_iActive(-1),
...
////////////////////////////////////////////////////////////////////////////////////////////////////
/// \fn	CListView::CListView(SallyAPI::GUI::CGUIBaseObject* parent, int x, int y, int width,
/// int height, int rows, int actionGraphic, int controlId) :SallyAPI::GUI::CForm(parent, x, y,
/// width, height, controlId), m_iStartPicture(0), m_iRows(rows), m_iActionGraphic(actionGraphic),
...
CListView::CListView(SallyAPI::GUI::CGUIBaseObject* parent, int x, int y, int width, int height, 
					int rows, int actionGraphic, int controlId)
	:SallyAPI::GUI::CForm(parent, x, y, width, height, controlId), m_iStartPicture(0),
	m_iRows(rows), m_iActionGraphic(actionGraphic), m_iActive(-1), m_bUseImageList(false),
...
void CListView::CreateListView()
{
	...
	m_iCols = (m_iHeight / LISTVIEW_ITEM_HEIGHT) - 1;
	if (m_iCols < 0)
		m_iCols = 0;
	...
	for (int i = 0; i < m_iCols; ++i)
	{
		...
		m_mButtonItem[i] = new SallyAPI::GUI::CListViewButton(this, (m_iRows * LISTVIEW_ITEM_HEIGHT),
			(i * LISTVIEW_ITEM_HEIGHT) + LISTVIEW_ITEM_HEIGHT, m_iWidth - (m_iRows * LISTVIEW_ITEM_HEIGHT) - CONTROL_HEIGHT,
...
void CListView::SetStartItem(int startItem)
{
	...
	if (startItem + m_iCols > (int) m_vItems.size())
		startItem = m_vItems.size() - m_iCols + 1;
	if (startItem <= 0)
		startItem = 0;
...
int CListView::GetMaxItemStart()
{
	...
	int maxItemStart = playlistSize - m_iCols;
...
bool CListView::RemoveItem(int index)
{
	...
	while (iter < m_vItems.end())
	{
		if (i == index)
		{
			...
			if (((int) m_vItems.size() < m_iStartPicture + m_iCols) && (m_iStartPicture > 0))
				--m_iStartPicture;

			if ((int) m_vItems.size() <= m_iCols - 1)
				ResetListView();
...
void CListView::ResetListView()
{
	...
	m_mButtonItem[m_iCols -1]->SetAlphaBlending(0);
	if (m_iRows > 0)
	{
		m_mButtonAction[0]->SetAlphaBlending(255);
		m_mButtonAction[m_iCols -1]->SetAlphaBlending(0);
	}
}
...
// 		OutputDebugStr(ttt.c_str());
	...
	while ((yTemp + moveValue <= 0) && (m_iStartPicture + m_iCols < (int) m_vItems.size() + 1))
	{
		for (int k = 0; k < m_iCols; ++k)
		{
			m_mButtonItem[k]->SetNumber(m_mButtonItem[k]->GetNumber() + 1);
...
// 		OutputDebugStr(ttt.c_str());
	...
	if (m_iStartPicture + m_iCols >= (int) m_vItems.size() + 1)
	{
		int newY = m_mButtonItem[0]->GetPositionY() + moveValue;
		if (newY < LISTVIEW_ITEM_HEIGHT)
		{
	...
	m_mButtonItem[m_iCols - 1]->SetAlphaBlending(255 / CONTROL_HEIGHT * ((m_iCols * LISTVIEW_ITEM_HEIGHT) - m_mButtonItem[m_iCols - 1]->GetPositionY()));
	if (m_iRows > 0)
	{
		m_mButtonAction[m_iCols - 1]->SetAlphaBlending(255 / CONTROL_HEIGHT * ((m_iCols * LISTVIEW_ITEM_HEIGHT) - m_mButtonItem[m_iCols - 1]->GetPositionY()));
	}
}
...
void CListView::UpdateView()
{
	...
	int maxPositon = (m_vItems.size() + 2 - m_iCols - 1) * CONTROL_HEIGHT;
