////////////////////////////////////////////////////////////////////////////////////////////////////
/// \file	MediaPlayer\LoadImage.cpp
///
...
void CLoadImage::Load()
{
	...
	for (int i = startItem; (i <= endItem) && (!m_bPleaseStop) && (startViewTemp == *m_piStartView) && (i < (int) m_vpAlbumList->size()); ++i)
	{
...
void CLoadImage::UnLoad()
{
	...
	// delete before
	for (int i = 0; (i < startItem) && (i < (int) m_vpAlbumList->size()); ++i)
	{
	...
	// delete at the end
	for (int i = endItem + 1; (i < (int) m_vpAlbumList->size()); ++i)
	{
		m_pParentControl->SendMessageToParent(0, 0, GUI_APP_RENDER_LOCK);
